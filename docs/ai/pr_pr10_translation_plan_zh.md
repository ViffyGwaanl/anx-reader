...
2) 全文翻译（阅读页内联显示）：要支持“仅显示翻译”和“双语显示”。
   - 这件事 **已经在 foliate-js 里通过 TranslationMode 支持**（`TRANSLATION_ONLY` / `BILINGUAL`），我们要保证全文翻译返回的字符串是“干净译文”。
   - **只翻译当页 + 下页**（限制预取范围，避免 API 风暴与不必要的翻译）。
   - **只做显示，不修改 EPUB 文件内容**：翻译通过 DOM 追加 `.translated-text` 元素呈现。
3) AI 提问/工具获取的章节内容必须是**原文**：不得把 `.translated-text` 混入 `getChapterContent()` / `getPreviousContent()` 等提取结果。
4) 提交模式（导出时）：`SubmitKind.REPLACE` / `SubmitKind.APPEND_BLOCK` **可选**。
5) 翻译结果要 **可缓存且可复用**（至少跨 session 的持久缓存），退出/重进能恢复体验（命中缓存不再走网络）。
6) **每本书**都要支持“清除全文翻译缓存”。
7) 做完“全量翻译”后可 **导出** 成翻译好的 EPUB。
8) 翻译提示词要可自定义（且全文翻译 prompt 与选中翻译 prompt **必须不同**）。
9) 模型要能单独指定“翻译专用模型”（不影响聊天默认模型）。
...
- 当 Full-text Translation Service 选择 AI 时：
  ...
  - 保留换行（段落结构）尽量接近原文。
  - **只翻译当页 + 下页**（不做更多页的预翻译）。
  - **只做显示，不修改 EPUB**（不会写回、不会改章节内容源数据）。
- AI 提问/工具读取内容必须为**原文**：`getChapterContent`/`getPreviousContent` 等需要过滤 `.translated-text`。
- 有并发控制（防止快速滚动导致 API 风暴）。
- 有持久缓存：同样的文本块再次出现时不会再次请求；退出/重进命中缓存可恢复。
- 支持按书清理缓存（每本书一个“清除全文翻译缓存”入口）。
